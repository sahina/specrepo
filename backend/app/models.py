...
class APISpecification(Base):
    ...
    openapi_content = Column(
        JSON, nullable=False
    )  # Consider using JSONB for PostgreSQL
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    updated_at = Column(DateTime(timezone=True), onupdate=func.now())
...
class MockConfiguration(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    api_specification_id = Column(
        Integer, ForeignKey("api_specifications.id"), nullable=False
    )
    wiremock_mapping_json = Column(
        JSON, nullable=False
    )  # Consider JSONB for PostgreSQL
    deployed_at = Column(DateTime(timezone=True), server_default=func.now())
    status = Column(String)  # e.g., 'active', 'inactive', 'error'

    api_specification = relationship(
        "APISpecification", back_populates="mock_configurations"
    )
...
class ValidationRun(Base):
    ...
    api_specification = relationship(
        "APISpecification", back_populates="validation_runs"
    )
